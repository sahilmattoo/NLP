...
path_for_dataset = r'C:\Users\smattoo5\Desktop\GLIM\TwitterSentimentAnalysis_Flask_Pipeline\Hate-Speech-Classification-deployed-using-Flask\dataset'

filename = path_for_dataset+"//modified.csv"
data = pd.read_csv(filename)

def readData(path):
    #filename = path+"//twitter_sentiments.csv"
    filename = path+"//modified.csv"
    data = pd.read_csv(filename)
    return data
...
def buidPipeline(vectorizer, model, data):
    ...
    dump(pipeline, filename="text_classification_spyder_Modified.joblib")
